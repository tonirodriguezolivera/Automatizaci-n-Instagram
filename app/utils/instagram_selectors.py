# app/utils/instagram_selectors.py
# Mantiene la misma semántica que GramAddict, para facilitar el port.
# Puedes cambiar IG_APP_ID si usas un build distinto de Instagram.
IG_APP_ID = "com.instagram.android"


class ResourceID:
    def __init__(self, APP_ID: str = IG_APP_ID):
        ...
        self.ROW_PROFILE_HEADER_FOLLOWERS_CONTAINER = (
            f"{APP_ID}:id/row_profile_header_followers_container|{APP_ID}:id/row_profile_header_container_followers"
        )
        self.ROW_PROFILE_HEADER_FOLLOWING_CONTAINER = (
            f"{APP_ID}:id/row_profile_header_following_container|{APP_ID}:id/row_profile_header_container_following"
        )
        ...
        self.USER_LIST_CONTAINER = (
            f"{self.FOLLOW_LIST_CONTAINER}|{self.ROW_USER_CONTAINER_BASE}|"
            f"{self.RECOMMENDED_USER_ROW_CONTENT_IDENTIFIER}"
        )
        self.SEARCH_ROW_ITEM = (
            f"{self.ROW_SEARCH_USER_USERNAME}|{self.ROW_HASHTAG_TEXTVIEW_TAG_NAME}|"
            f"{self.ROW_PLACE_TITLE}"
        )
